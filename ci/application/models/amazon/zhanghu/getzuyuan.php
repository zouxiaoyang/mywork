<?php
	class Getzuyuan extends CI_Model{
...
//从数据库中拉取数据;	
	function getzuyuan_message($arr_zhanghu,$value,$cur_page,$per_page,$choose){
...
//输出表头； 
		$file = fopen("./tools/resource/download_export/contacts_zuyuan.csv","a");
...
//							$arr_pic = array_filter($arr_pic);
			...
			if($segs){
				$showtable.='<td><a href="/amazon/edit/edit_table/?&page='.$segs.'&order_id='.$_SESSION['TIME'].'">编辑</a></td>';
			}else{
				$showtable.='<td><a href="/amazon/edit/edit_table/?&order_id='.$_SESSION['TIME'].'">编辑</a></td>';
			}
			$showtable.='</tr>';
		}
		$showtable.='</table>';
