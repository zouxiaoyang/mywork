...
<p><h4 style="text-align:center">订单展示页面</h4></p>
	<ul id="shaixuan">
			<li><?php echo anchor('/amazon/choose/showzuyuan/getallorder','显示全部订单');?></li>
			<li><?php echo anchor('/tools/resource/download_export/contacts_zuyuan.csv','导出订单表格')?></li>
...
function closeDiv(){
		...
		$('date_title').onclick=function(){
			var date = $('time').value
				if(!date){
					...
					var url = '/amazon/choose/showzuyuan/gettime/';
	...
	//				为了保持页码的一致性！		
						...
						window.location.href="/amazon/choose/showzuyuan/showmessage"
...
//		账户筛选;
	...
	$('shaixuan_zhanghu').onchange=function(){
				...
				if(zhanghu!='账户选择'){
					...
					var url = '/amazon/choose/showzuyuan/getpost/';
